...
class RayPPOTrainer:
    ...
    def fit(self):
        ...
        # --- Memory debugging (opt-in via VERL_MEM_DEBUG=1) ---
        _mem_debug = os.environ.get("VERL_MEM_DEBUG", "0") == "1"
        _mem_tracker = None
        if _mem_debug:
            from debug.mem_hooks import MemoryTracker

            _mem_log_dir = os.path.join(self.config.trainer.default_local_dir, "mem_debug")
            _mem_step_file = os.path.join(_mem_log_dir, "current_step.txt")
            _mem_tracker = MemoryTracker(
                log_dir=_mem_log_dir,
                step_file=_mem_step_file,
                enable_tracemalloc=os.environ.get("VERL_MEM_TRACEMALLOC", "1") == "1",
            )
            print(f"[fit] Memory debugging ENABLED — logs in {_mem_log_dir}")
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if _mem_tracker is not None:
                    _mem_tracker.snapshot("step_start", step=self.global_steps)
                ...
                with marked_timer("step", timing_raw):
                    ...
                    if _mem_tracker is not None:
                        _mem_tracker.snapshot("after_generate", step=self.global_steps)
                    ...
                    if _mem_tracker is not None:
                        _mem_tracker.snapshot("after_union_gen", step=self.global_steps)
                        _mem_tracker.log_batch_info("after_union_gen", step=self.global_steps, batch=batch)
                    ...
                    if _mem_tracker is not None:
                        _mem_tracker.snapshot("after_reward", step=self.global_steps)
                    ...
                    if _mem_tracker is not None:
                        _mem_tracker.snapshot("after_update_actor", step=self.global_steps)
                ...
                if (
                    ...
                    if _mem_tracker is not None:
                        _mem_tracker.snapshot("after_validate", step=self.global_steps)
                ...
                if _mem_tracker is not None:
                    _mem_tracker.snapshot("step_end", step=self.global_steps)
                    _mem_tracker.gc_collect_and_log("step_end", step=self.global_steps)
